...
clear
# Print text in red
echo -e "${blue}Welcome to the installation for PaperMC!"

# Get the version of PaperMC to install
read -p "Enter a version: " version
# Download the requested version
case $version in
    1.18.2)
        echo "Downloading Paper 1.18.2..."
        wget https://papermc.io/api/v1/paper/1.18.2/latest/download -O paper-1.18.2.jar
        echo "Creating the Paper 1.18.2 folder..."
        mkdir paper-1.18.2
        echo "Moving the Paper 1.18.2 jar to the folder..."
        mv paper-1.18.2.jar paper-1.18.2/
    ...
    1.17.1)
        ...
        wget https://papermc.io/api/v1/paper/1.17.1/latest/download -O paper-1.17.1.jar
        echo "Creating the Paper 1.17.1 folder..."
        mkdir paper-1.17.1
        echo "Moving the Paper 1.17.1 jar to the folder..."
        mv paper-1.17.1.jar paper-1.17.1/
        echo "Would you like a run.sh file with Aikar's flags to improve performance? (y/n)"
        read answer
        # If the user wants a run.sh file
        if [ $answer = "y" ]; then
